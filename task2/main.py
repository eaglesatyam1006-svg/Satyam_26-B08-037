...
def process_image(img_path, output_path):
    ...
    # Step A: Grayscale + blur + edge detection
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    blur = cv2.GaussianBlur(gray, (5, 5), 0)
    edges = cv2.Canny(blur, 50, 150)

    # Step B: Region of interest — only look at bottom half of image
    ...
    # Step C: Detect straight lines using Hough Transform
    ...
    if lines is not None:
        for line in lines:
            ...
            if x2 - x1 == 0:
                continue
            slope = (y2 - y1) / (x2 - x1)
            # Separate left lane (negative slope) vs right lane (positive slope)
            if slope < -0.3:
    ...
    # Step D: Fill drivable area between the two lanes
